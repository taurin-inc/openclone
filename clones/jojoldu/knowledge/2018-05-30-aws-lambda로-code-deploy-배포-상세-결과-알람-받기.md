---
topic: "AWS Lambda로 Code Deploy 배포 상세 결과 알람 받기"
source_type: other
source_url: "https://jojoldu.tistory.com/298"
authorship: self
published_at: 2018-05-30
---
thoughtful search steps:

1.  **Analyze the User Request:**
    *   Goal: Extract the main article content from the provided text.
    *   Specific exclusions: Comments, related posts, ads, sidebar widgets, subscription buttons, share buttons, author bio section at bottom.
    *   Specific inclusions: Title, main content, author name, publish date.
    *   Format: Markdown.
    *   Language: Preserve original (Korean).

2.  **Examine the Original Content:**
    *   *Title:* AWS Lambda로 Code Deploy 배포 상세 결과 알람 받기
    *   *Author:* 향로 (기억보단 기록을)
    *   *Publish Date:* 2018. 5. 30.
    *   *Category:* DevOps (Part of header/path, worth noting).
    *   *Main Content Sections:*
        *   Introduction: Context about Code Deploy, TravisCI, and the difficulty of getting detailed alarms.
        *   1. AWS Lambda 소개 (Introduction to Lambda).
        *   2. 본문 (The main tutorial/implementation).
            *   2-1. AWS Lambda 생성 (Creating Lambda).
            *   2-2. AWS SNS 연결 (Connecting SNS).
            *   2-3. 테스트 (Testing).
            *   2-4. AWS Code Deploy 연결 (Connecting Code Deploy).
            *   2-5. 실제 테스트 (Actual test results).
        *   마무리 (Conclusion).
        *   참고 (References).
    *   *Sections to Remove:*
        *   [본문 바로가기] links.
        *   "반응형" placeholders.
        *   Ads/Widgets (implied by placeholders like "반응형").
        *   Share buttons (URL 복사, 카카오톡 공유, etc.).
        *   Author bio section at the bottom (everything from "좋은 애플리케이션 구현 & 설계..." down).
        *   Comments (the Giscus section).
        *   Tistory toolbar/Shortcuts info at the very bottom.

3.  **Structure the Markdown Output:**

    *   Start with the Title.
    *   Add metadata (Author and Date).
    *   Process the body text, converting headers to markdown (`#`, `##`, `###`).
    *   Handle code blocks (keep the JavaScript code).
    *   Handle images (keep the markdown syntax for images).
    *   Handle lists and links.

4.  **Drafting the Content (Mental or scratchpad):**
    *   Title: `# AWS Lambda로 Code Deploy 배포 상세 결과 알람 받기`
    *   Meta: `Author: 향로 (기억보단 기록을) | Date: 2018. 5. 30.`
    *   Intro: "Code Deploy와 다른 CI 툴..."
    *   ... (Extract text and format code snippets) ...
    *   Conclusion: "AWS 초보자이지만..."
    *   Reference link.

5.  **Refinement/Cleanup:**
    *   Ensure all "Related Posts" and "Tags" are gone.
    *   Ensure the code block is correctly formatted with ` ```javascript `.
    *   Check for any trailing "Subscription" or "Admin" UI text and delete it.

6.  **Final Review against Constraints:**
    *   Only main article content? Yes.
    *   Removed comments? Yes.
    *   Removed related posts? Yes.
    *   Removed ads/widgets/buttons? Yes.
    *   Removed bio? Yes.
    *   Kept Title/Content/Author/Date? Yes.
    *   Markdown format? Yes.
    *   Preserved language? Yes.# AWS Lambda로 Code Deploy 배포 상세 결과 알람 받기
...
![텔레그램1](https://t1.daumcdn.net/cfile/tistory/9959273B5B0DDA0E28)

알람으로 온 링크를 클릭해서 들어가봐도 알 수 있는 내용은 없습니다.

![텔레그램2](https://t1.daumcdn.net/cfile/tistory/991D923E5B0DDA0F29)
...
![람다1](https://t1.daumcdn.net/cfile/tistory/99F8A34A5B0DDA0D27)

![람다2](https://t1.daumcdn.net/cfile/tistory/99F5F33F5B0DDA0F23)

![람다3](https://t1.daumcdn.net/cfile/tistory/9983F53D5B0DDA0F14)

좀 더 아래로 내려가보시면 실제 람다가 수행할 코드를 입력하는 화면이 나옵니다.

![람다4](https://t1.daumcdn.net/cfile/tistory/991A4E335B0DDA0E25)
...
![람다5](https://t1.daumcdn.net/cfile/tistory/993A1F4B5B0DDA0E27)
...
![람다6](https://t1.daumcdn.net/cfile/tistory/992286365B0DDA0F27)

여기서 본인이 원하는 채널을 선택하신뒤 **Add Incoming WebHooks integration** 을 클릭합니다.

![람다7](https://t1.daumcdn.net/cfile/tistory/9963F7405B0DDA0F27)

그러면 본인의 webhook url이 나오는데, 여기서 `/services`부터 복사해서 AWS 람다 환경변수에 등록하시면 됩니다.

![람다8](https://t1.daumcdn.net/cfile/tistory/998CA6455B0DDA0F2C)

최종적으로 AWS 람다는 다음과 같이 Nodejs 코드와 환경변수가 등록되어있으면 됩니다.

![람다9](https://t1.daumcdn.net/cfile/tistory/995168395B0DDA0F28)
...
![sns1](https://t1.daumcdn.net/cfile/tistory/99B3344E5B0DDA0E27)

여기서 주제 이름과 표시 이름을 등록합니다.

(표시 이름은 10자 제한이 있습니다.)

![sns2](https://t1.daumcdn.net/cfile/tistory/9968A2385B0DDA0E24)

이렇게만 하면 생성이 끝납니다!

![sns3](https://t1.daumcdn.net/cfile/tistory/99935B3D5B0DDA0D2E)

생성한 SNS를 람다에 연결하겠습니다.

좀전에 생성한 AWS 람다 페이지로 가시면 상단 좌측에 SNS 탭이 있습니다.

![sns4](https://t1.daumcdn.net/cfile/tistory/99F0ED335B0DDA0F26)

SNS 탭을 클릭하시면 하단에 SNS 등록 페이지가 열리는데요.

방금 생성한 SNS를 추가하면 됩니다.

![sns5](https://t1.daumcdn.net/cfile/tistory/994C8D3B5B0DDA0F27)

추가되면 아래처럼 SNS와 연결된 것을 바로 확인할 수 있습니다.

![sns6](https://t1.daumcdn.net/cfile/tistory/9931993F5B0DDA0F1D)

SNS에서도 AWS 람다와 연결된 것을 확인할 수 있습니다.

![sns7](https://t1.daumcdn.net/cfile/tistory/991A854E5B0DDA0F25)
...
![test1](https://t1.daumcdn.net/cfile/tistory/9952CE345B0DDA0F24)

여기서 **새로운 테스트 이벤트 생성** 을 선택하시고, 이벤트 템플릿은 아무거나 선택하시면 됩니다. (어차피 새로 입력합니다.)

![test2](https://t1.daumcdn.net/cfile/tistory/991DC7395B0DDA0E22)
...
![test3](https://t1.daumcdn.net/cfile/tistory/9959144D5B0DDA0F1B)

그럼 아래처럼 실행한 결과가 노출 됩니다.

![test4](https://t1.daumcdn.net/cfile/tistory/999564465B0DDA0F27)

실제로 슬랙에도 다음처럼 아주 이쁘게 테스트 메세지가 노출됩니다.

![test5](https://t1.daumcdn.net/cfile/tistory/9934D44B5B0DDA0E27)
...
![codedeploy1](https://t1.daumcdn.net/cfile/tistory/99A4FF395B0DDA0D29)

편집 페이지에서 고급 -> 트리거 -> 트리거 생성을 차례로 클릭합니다.

![codedeploy2](https://t1.daumcdn.net/cfile/tistory/99DF69355B0DDA0E29)

그리고 아래처럼 대상 이벤트로 Deployment succeeds, Deployment fails를 선택하시고, SNS는 방금 생성한 SNS를 선택하면 끝납니다.

![codedeploy3](https://t1.daumcdn.net/cfile/tistory/9982123E5B0DDA0E27)
...
![결과_성공](https://t1.daumcdn.net/cfile/tistory/996C67435B0DDA0E2D)

이렇게 배포 링크 (Deployment Link)와 기타 정보들이 잘 노출됩니다.

반대로 배포가 실패하게 되면!

![결과_실패](https://t1.daumcdn.net/cfile/tistory/9974D33C5B0DDA0F26)

Error Code, Error Message로 에러 메세지가 잘 노출되는걸 확인할 수 있습니다.

메세지의 링크를 클릭해보면

![결과_링크](https://t1.daumcdn.net/cfile/tistory/99D1AC4B5B0DDA0F29)
