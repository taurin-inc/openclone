---
topic: "SpringBoot에서 날짜 타입 JSON 변환에 대한 오해 풀기"
source_type: other
source_url: "https://jojoldu.tistory.com/361"
authorship: self
published_at: 2018-12-17
---
thoughtful memo:

*   Goal: Extract main article content from a provided blog post.
    *   Requirement: Remove comments, related posts, ads, sidebar widgets, subscription buttons, share buttons, author bio at the bottom.
    *   Keep: Title, main content, author name, publish date.
    *   Format: Markdown.
    *   Language: Preserve original (Korean).

    *   *Title:* SpringBoot에서 날짜 타입 JSON 변환에 대한 오해 풀기
    *   *Author:* 향로 (기억보단 기록을)
    *   *Date:* 2018. 12. 17.
    *   *Introduction:* Greeting, Github link reference.
    *   *Section 0 (Introduction):* Context about Spring, DTOs, LocalDate/LocalDateTime. Mentioning development environment (Spring Boot 1.5.17 vs 2.0).
    *   *Section 1 (Request Parameter):* Covers `@ModelAttribute` and `@RequestParam`. Experiments with `@DateTimeFormat`.
    *   *Section 2 (Request Body):* Covers POST requests with `@RequestBody`. Experiments with `@DateTimeFormat` vs `@JsonFormat`.
    *   *Section 3 (Response Body):* Covers returning JSON from controller. Experiments with `@JsonFormat`.
    *   *Section 4 (@DateTimeFormat vs @JsonFormat):* Explanation of the difference (Spring vs Jackson).
    *   *Section 5 (SpringBoot 2.x vs 1.x):* JSR 310 dependency differences.
    *   *Section 6 (Conclusion):* Summary of findings.

    *   Remove "본문 바로가기" (Link to content).
    *   Remove "반응형" (Responsive ad placeholders).
    *   Remove social share icons/buttons.
    *   Remove author bio section/subscription section (everything after the conclusion).
    *   Remove comment section ("반응 0개", "댓글 10개", etc.).
    *   Remove Tistory toolbar and shortcut keys help.

    *   Ensure headers use appropriate markdown levels (`#`, `##`, `###`).
    *   Format code blocks correctly.
    *   Maintain image links.# SpringBoot에서 날짜 타입 JSON 변환에 대한 오해 풀기
...
![1-1](https://t1.daumcdn.net/cfile/tistory/99978F345C17048C29)
...
![1-2](https://t1.daumcdn.net/cfile/tistory/99D91B445C17048B2E)
...
![1-3](https://t1.daumcdn.net/cfile/tistory/9965D2505C17048B2C)
...
![1-4](https://t1.daumcdn.net/cfile/tistory/99F9BF3D5C17048B03)
...
![1-5](https://t1.daumcdn.net/cfile/tistory/994156355C17048C2D)
...
![1-6](https://t1.daumcdn.net/cfile/tistory/9954A6365C17048B25)

테스트가 성공합니다.

![1-7](https://t1.daumcdn.net/cfile/tistory/997930425C17048B28)
...
![2-1](https://t1.daumcdn.net/cfile/tistory/999A294A5C17048C2B)

당연히 실패합니다.

자 그럼 Get에서 사용한 `@DateTimeFormat`을 사용해보겠습니다.

![2-2](https://t1.daumcdn.net/cfile/tistory/99C74B395C17048C24)

다시 테스트를 돌려보면!

![2-3](https://t1.daumcdn.net/cfile/tistory/9996BF4C5C17048C2C)
...
![2-4](https://t1.daumcdn.net/cfile/tistory/993A61485C17048C29)

오! Get과 다르게 `@JsonFormat`으로 성공합니다.

![2-5](https://t1.daumcdn.net/cfile/tistory/99ACA9355C17048B29)
...
![2-6](https://t1.daumcdn.net/cfile/tistory/99A863465C17048C2A)

그러면 성공합니다!

반대로 `@JsonFormat`을 틀리게 설정하고 테스트해보면!

![2-7](https://t1.daumcdn.net/cfile/tistory/99765A395C17048C27)
...
![3-1](https://t1.daumcdn.net/cfile/tistory/99AEF24D5C17048B29)

당연히 실패합니다.

자 그럼 이번엔 `@DateTimeFormat`을 선언한 뒤, 다시 테스트를 해봅니다.

![3-2](https://t1.daumcdn.net/cfile/tistory/99781C3B5C17048C1C)

1, 2번과 달리 `@DateTimeFormat` 에서 실패합니다!

![3-3](https://t1.daumcdn.net/cfile/tistory/996EB4395C17048C1E)

그럼 `@JsonFormat` 에서는?

![3-4](https://t1.daumcdn.net/cfile/tistory/9947404D5C17048C2C)
...
![jackson](https://t1.daumcdn.net/cfile/tistory/99A22E3D5C17048C0A)
...
![jsr](https://t1.daumcdn.net/cfile/tistory/9979D8405C17048C28)
