---
topic: "is_left vs left_at vs left_status: 상황에 맞는 DB 상태값 설계"
source_type: other
source_url: "https://jojoldu.tistory.com/577"
authorship: self
...
RDBMS의 버전과 종류에 따라 다르지만, `is not null`은 **인덱스를 타지 못할때가 많다**.
...
만약 **아직 탈퇴하지 않는 계정** 을 조회하는 기능을 구현한다고 하면, 이 기능에서 **timestamp만 유일하게 인덱스를 못탈 수도 있는** 쿼리가 발생한다.
