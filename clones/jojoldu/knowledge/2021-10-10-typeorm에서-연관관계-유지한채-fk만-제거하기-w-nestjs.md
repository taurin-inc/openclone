---
topic: "TypeORM에서 연관관계 유지한채 FK만 제거하기 (w. NestJS)"
source_type: other
source_url: "https://jojoldu.tistory.com/605"
authorship: self
...
다만, FK가 있을 경우 테이블 복사, OnlineDDL를 비롯해서 클러스터링 / 샤딩 / 파티셔닝 등 여러 **대량의 데이터 상황에서 제약들이 발생** 하여서 일정 규모 이상의 서비스에서는 DBA분들의 요청으로 FK를 제거해서 사용하기도 합니다.
...
* `nullable: false`가 없을 경우 **FK임에도 nullable 컬럼** 이 생성됩니다.
...
이렇게 테스트 코드를 작성후 검증을 해보면 Lazy Loading 에서도 FK 제약 조건 없이 잘 작동하는 것을 확인할 수 있습니다.
