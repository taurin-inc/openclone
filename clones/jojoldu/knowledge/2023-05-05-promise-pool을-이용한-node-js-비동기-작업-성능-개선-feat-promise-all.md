---
topic: "Promise Pool을 이용한 Node.js 비동기 작업 성능 개선 (feat. Promise.all)"
source_type: other
source_url: "https://jojoldu.tistory.com/714"
authorship: self
...
(실제 운영 환경에서는 chunk를 50개로도 잡지 않는다. 다른 서버들도 여러 쿼리들이 수행될 수 있기 때문에 그보다 훨씬 더 낮은 값으로 잡는다.)

예로 든 것처럼 Chunk단위로 나눠진 Prmise 배열들을 `Promise.all`로 수행하는 형태는 다음과 같다.

![promiseall](https://blog.kakaocdn.net/dna/bxCiAx/btsdZhWnx67/AAAAAAAAAAAAAAAAAAAAAFDH5g_gJPrv7M-baAEn2skWvNTR6UuAlqSjbjpQcNFy/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=WNy7pfcTKeQJ12x1BsdaOO8e4SU%3D)

각 Chunk 단위로 병렬로 Promise들이 수행되어 효율적으로 기능을 수행할 수 있다.

Chunk + `Promise.all` 의 조합에서는 **Promise 배열 내 가장 긴 Promise가 Chunk의 수행 시간** 이 된다.

![promiseall2](https://blog.kakaocdn.net/dna/bCJCfM/btsd3eKugxc/AAAAAAAAAAAAAAAAAAAAAM8RJlfRsE3S8DbwD6qFjifveHU7kSHRtHQ9FfyXrHzY/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=cxaimSj2atPICMdCYVyYh0iQB%2Fk%3D)
...
![promisepool](https://blog.kakaocdn.net/dna/cjdU6x/btsdZrLi9tf/AAAAAAAAAAAAAAAAAAAAAGZQm2bs45sThkKAf02FRnNSR_xNpCp6CGQsF30mT-rp/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=dBcYUJ1dSI30%2BgPzSPi%2BpLfp0OA%3D)
...
- chunk 단위는 50으로 설정했다.
  ...
  - `measurePromisePool` 에서는 **50개의 레일** 을 깔아 Promise 1000개를 수행한다.

이를 수행해보면?

Promise.all은 각 Chunk에서의 최소 수행시간이 0.5, 최대 수행시간이 1.9초인 것을 알 수 있다.

![chunk](https://blog.kakaocdn.net/dna/xiIoy/btsd0R3jt0y/AAAAAAAAAAAAAAAAAAAAAPFCmQIHhbWyDkJ2Ep7qYSywF5Lv70MiGq6ONAO69aEo/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=O%2FmMXj8lIZGJy6Y7GWzzDOJzgy8%3D)

그래서 **각 Chunk 내에서 가장 긴 작업 시간이 Chunk의 작업 시간** 이 된다.

전체를 모두 수행한 2개 작업들의 최종 결과는 다음과 같다.

![result](https://blog.kakaocdn.net/dna/cw95CE/btsd00Z3y3W/AAAAAAAAAAAAAAAAAAAAAG1bnzNkq3SSIru3mRuGgr-DIIWbNBYB8JelpvGMQ4UR/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=4ezhPTZW7yjeagMtIMLf1gV%2FFek%3D)
...
Chunk 단위마다 긴 작업이 하나씩은 꼭 포함되어있기 때문에 이런 결과가 나온 것이지, **Promise마다 작업시간이 거의 유사한 상황에서는 결과가 큰 차이가 없다**.
