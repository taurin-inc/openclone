---
topic: "JobParameter 활용 방법 (feat. LocalDate 파라미터 사용하기)"
source_type: other
source_url: "https://jojoldu.tistory.com/490"
authorship: self
published_at: 2020-03-30
---
# JobParameter 활용 방법 (feat. LocalDate 파라미터 사용하기)

**by 향로 (기억보단 기록을) 2020. 3. 30.**
...
기존에 사용하던 방식은 무엇인지, 해당 방식에 어떤 단점이 있는지, 개선할 수 있는 방법은 무엇인지 등등을 정리해보았으니 Job Parameter를 사용하면서 불편하셨던 분들은 참고하시면 좋을것 같습니다.
...
![1](https://t1.daumcdn.net/cfile/tistory/99D834465E81C90C14)
...
![1-1](https://t1.daumcdn.net/cfile/tistory/992EE54C5E81C90C0E)

![1-2](https://t1.daumcdn.net/cfile/tistory/999506435E81C90C01)
...
![2](https://t1.daumcdn.net/cfile/tistory/99D8AC3A5E81C90C19)
...
Spring의 `@Value` 어노테이션은 SpEL (Spring Expression Language) 을 지원합니다. 그 중 `T` 코드를 이용하여 **지정된 클래스의 static Method** 를 SpEL 내부에서 호출할 수가 있습니다.

![4](https://t1.daumcdn.net/cfile/tistory/99DDD13B5E81C90C05)
...
만약 별도의 포맷을 써야 할 경우라면 별도의 Converter 클래스를 사용하기를 추천드립니다.
...
이렇게 만든 Converter는 다음과 같이 호출합니다.
...
LocalDate와 같이 기본적으로 Spring Batch가 지원하지 않는 타입에 대해 좀 더 잘 관리할 수 있는 방법을 배워보았습니다. 이 3가지 중에 팀 혹은 개인에 맞는 방법을 선택하시면 될 것 같습니다.

> ps. 저 같은 경우엔 Setter 혹은 Constructor를 선호합니다. 필드 인잭션의 경우 Spring Context가 없는 테스트 코드 작성시 JobParameter 값 변조가 어렵기도 하고 SpEL 문법에 어색한 분들이 계실수 있기 때문입니다.
