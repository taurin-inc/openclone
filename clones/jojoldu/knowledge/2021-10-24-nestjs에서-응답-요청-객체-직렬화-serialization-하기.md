---
topic: "NestJS에서 응답/요청 객체 직렬화 (Serialization) 하기"
source_type: other
source_url: "https://jojoldu.tistory.com/610"
authorship: self
...
- `ValidationPipe` 에 추가 옵션으로 `{ transform: true }` 을 넣게 되면, 벨리데이션이 끝난 요청 객체를 실제 클래스의 인스턴스로 변환되어 Controller에서 받을 수 있게 됩니다.

그래서 `plainToClass` 없이 곧바로 인스턴스를 사용할 수 있습니다.
...
JSON 데이터 상에서는 이들은 **특정 포맷의 문자열** 값인데요. 이를 `@Transform` 을 통해 진행할 수 있습니다.
...
- `property.value` 를 추출해서 적절한 형태로 변환해서 반환합니다.

(2) `property.value + ''`
- `property.value` 의 값이 `any` 이므로, 범용성을 위해 문자열로 변환하여 처리합니다.
