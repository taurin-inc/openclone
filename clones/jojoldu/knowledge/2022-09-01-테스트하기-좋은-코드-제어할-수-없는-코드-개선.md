---
topic: "테스트하기 좋은 코드 - 제어할 수 없는 코드 개선"
source_type: other
source_url: "https://jojoldu.tistory.com/676"
authorship: self
...
`LocalDateTimw.now()`의 코드를 보면 알겠지만, 현재 시간은 항상 다르기 때문이다.

그렇기 때문에 **실행할때마다 항상 결과가 똑같은 형태** 의 메소드로 전환이 필요하다.
...
![layer](https://blog.kakaocdn.net/dna/b2DAKK/btrK4UgNxw2/AAAAAAAAAAAAAAAAAAAAAFMj046_h3RBOuG6IC8l00Bx4iF-Nsvi3og_xNsmUfhA/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=nUxjzFFwbxxGMzyLH3DbRjkh5pg%3D)
...
- 의존성 주입을 통해 제어할 수 없는 값을 다루는 객체들은 생성자 주입을 받아서 사용하거나 (2-3-2에서 자세히 소개한다.)
...
즉, 도메인 로직에서 `now()`를 메소드 인자로 받도록 변경했다면 이를 호출하는 Service, Repository 에서 해당 메소드의 인자로 `now()`를 넣어 주면 되는것인가? 그렇다면 Service, Repository의 테스트가 여전히 `now()`로 인해서 테스트가 어렵지 않은가?
...
- Service 역시 **가장 바깥쪽 계층이 아니기 때문에** 제어할 수 없는 값인 `now()`는 바깥에서 전달 받도록 한다. (Service의 테스트 코드 작성이 수월해야하기 때문이다.)
...
이 방식은 비단, 백엔드 코드에서만 적용되는 것은 아니다. 작성한 코드의 가장 바깥쪽이란 다음과 같다.

- AWS Lambda 등의 서버리스 환경에서의 `handler`
- 백엔드 API에서의 Controller
- 웹프론트 혹은 앱 등의 클라이언트에서의 이벤트 핸들러

즉, **프로그램의 진입점이 되는 영역** 에 최대한 테스트하기 어려운 코드를 모아두고, 그 다음 계층부터 테스트 하기 좋은 코드로만 이루어지도록 한다. 최대한 **제어할 수 없는 코드를 바깥으로 밀어내, 해당 코드에 의존하는 범위를 좁히는 것** 이다.
...
그래서 이런 문제를 의존성 주입 패턴으로 풀때가 많다. 제어할 수 없는 값을 반환하는 인터페이스를 두고, 이를 테스트와 메인코드에서 활용하는 것이다.

![time](https://blog.kakaocdn.net/dna/3FA2T/btrK9bg1O8Z/AAAAAAAAAAAAAAAAAAAAANjdf2Ii8kolUCBu3H4NYGG1vy9At0ZCYOIXHhsVc3dY/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1769871599&allow_ip=&allow_referer=&signature=I6NVsx%2FnX2lWOVQ4qam0Sk%2FfjuI%3D)
...
테스트 하기 좋은 코드가 될수록 우리의 코드는 부수효과가 없고, 항상 같은 결과가 반환되는 순수 함수가 될 수 있다. 테스트 코드 하기 좋은 코드로의 리팩토링은 결국 메인 코드의 품질을 높일 수 있다.
