---
topic: "Spring Batch ItemWriter 성능 비교"
source_type: other
source_url: "https://jojoldu.tistory.com/507"
authorship: self
published_at: 2020-07-08
---
thoughtful
*   Task: Extract only the main article content from the provided text.
    *   Exclusions: Comments, related posts, ads, sidebar widgets, subscription buttons, share buttons, author bio section at the bottom.
    *   Inclusions: Title, main content, author name, publish date.
    *   Format: Markdown.
    *   Language: Preserve the original language (Korean).

    *   *Title:* Spring Batch ItemWriter 성능 비교
    *   *Author:* 향로 (기억보단 기록을)
    *   *Date:* 2020. 7. 8.
    *   *Introduction:* Importance of batch performance (Reader vs Writer). Reference to Querydsl for Reader. Focus on JpaItemWriter improvements.
    *   *Test Environment:* AWS RDS Aurora, MacBook Pro specs, `rewriteBatchedStatements=true`.
    *   *Section 1: Merge vs Persist:*
        *   JPA Merge inefficiency.
        *   Spring Batch 4.2+ choice between Merge and Persist.
        *   1-1: Non Auto Increment comparison (Select + Insert vs Insert only). Performance results (1m 9s vs 2m 16s).
        *   1-2: Auto Increment comparison. Similar query behavior but architectural difference (Entity copying in Merge). Recommendation for Persist for new entities.
    *   *Section 2: Jdbc Batch Insert:*
        *   Why JpaItemWriter doesn't use Bulk Insert by default (Hibernate limitation with Auto Increment).
        *   Performance testing (Non Auto Increment & Auto Increment). Results: ~0.5s for 10k rows.
    *   *Final Comparison:* Comparison table and trade-offs between performance and maintainability (JPA vs JDBC).

    *   Remove navigation links (본문 바로가기).
    *   Remove "반응형" (Responsive) placeholders.
    *   Remove tags, related posts, subscription buttons, footer info, giscus comments, and Tistory toolbar info.

    *   Title: # Spring Batch ItemWriter 성능 비교
    *   Author/Date: by 향로 (기억보단 기록을) 2020. 7. 8.
    *   Sections with markdown headers and code blocks.
    *   Table formatting.
    ...
    *   Author? Yes.
    *   Date? Yes.
    *   Cleaned (no ads/sidebar/footer)? Yes.
    *   Markdown? Yes.
    *   Language preserved? Yes.# Spring Batch ItemWriter 성능 비교

by 향로 (기억보단 기록을) 2020. 7. 8.
...
> 모든 테스트는 Insert 합치기 옵션이 적용된 상태( Jdbc-url 설정에 `rewriteBatchedStatements` 옵션이 `true`) 에서 진행됩니다.
>
...
테스트 로그를 보면 Merge와 달리 **Insert쿼리만 발생** 한 것을 확인할 수 있습니다.
...
테스트 결과 **약 2배** 의 성능 차이가 발생하는 것을 확인할 수 있습니다.
...
위 코드를 실행해보면 **추가 Select 쿼리 없이** Insert만 수행되는 것을 확인할 수 있습니다.
...
Persist 테스트 코드를 수행해보면 마찬가지로 **Insert쿼리만 수행** 되는 것을 확인할 수 있습니다.
