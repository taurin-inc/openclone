---
topic: "PostgreSQL RDS Slow 쿼리 Slack으로 알람 보내기"
source_type: other
source_url: "https://jojoldu.tistory.com/570"
authorship: self
...
![intro](https://t1.daumcdn.net/cfile/tistory/99C468486096839A29)
...
- `log_statement`
  - 총 4개의 상태값을 가질 수 있음
  - `none`, `ddl`, `mod`, `all`
  - DDL (CREATE, ALTER 등) 에 대한 로그만 남기고 싶다면 `ddl`를 선택
- `log_min_duration_statement`
  - **지정된 시간 이상** 쿼리가 수행되면 로그를 남긴다
  - 단위는 ms / 즉, 1000로 지정할 경우 1초
  - 1000으로 할 경우 **1초이상 수행된 쿼리들은 모두 로그** 로 남게된다.
  - 위 log_statement와 중복으로 로그를 남기지 않는다.
  - 즉, log_statement로 DDL을 남기고, log_min_duration_statement 에서 1초이상 쿼리를 남기도록 하면 insert, update, delete, select 등에서는 1초이상 쿼리시 로그가 남는다.

![parameter](https://t1.daumcdn.net/cfile/tistory/996FA03A6096839A29)
...
![rds-log](https://t1.daumcdn.net/cfile/tistory/996C973A6096839A2A)
...
![slow-log](https://t1.daumcdn.net/cfile/tistory/99DC6D366096839A27)
...
![lambda1](https://t1.daumcdn.net/cfile/tistory/99DC72366096839A29)
...
![lambda-test](https://t1.daumcdn.net/cfile/tistory/994A0E346096839A28)
...
![lambda-test-data](https://t1.daumcdn.net/cfile/tistory/990E93336096839A29)
...
![lambda1](https://t1.daumcdn.net/cfile/tistory/990E89336096839A29)![lambda2](https://t1.daumcdn.net/cfile/tistory/99742E436096839A28)
...
![slack1](https://t1.daumcdn.net/cfile/tistory/996F963A6096839A29)
...
![cloudwatch1](https://t1.daumcdn.net/cfile/tistory/995328466096839A29)

구독 필터 생성 화면에 들어가시게 되면, 2-1에서 만든 Lambda 함수를 대상에 등록합니다.

![cloudwatch2](https://t1.daumcdn.net/cfile/tistory/996F9B3A6096839A29)

아래로 내려가보시면 로그형식이 나오는데, 여기서 로그형식은 기타를 선택합니다.

구독 필터 패턴의 경우 필요하신 형태에 따라 넣어주시면 됩니다.

**정규표현식이 아직 미지원** 하여서 [클라우드워치 로그 전용 패턴](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/FilterAndPatternSyntax.html) 을 쓰셔야 합니다.

저같은 경우에는 `"LOG" "duration"`로 등록하여 duration 메세지가 포함된 로그들만 사용하도록 했습니다.

구독 필터 이름이 없을 경우 생성이 불가능하니 꼭 식별 가능한 이름으로 입력합니다.

![cloudwatch3](https://t1.daumcdn.net/cfile/tistory/9981FE356096839A28)

기존의 데이터에 대한 패턴 테스트까지 끝나셨다면 **스트리밍 시작** 버튼을 클릭하여 구독을 활성화 시킵니다.

![cloudwatch4](https://t1.daumcdn.net/cfile/tistory/999CA8376096839A2A)

그럼 아래와 같이 로그그룹의 구독 필터에 1개의 구독이 추가된 것을 볼 수 있습니다.

![cloudwatch5](https://t1.daumcdn.net/cfile/tistory/997433436096839A28)
...
![slack2](https://t1.daumcdn.net/cfile/tistory/99C463486096839A28)
