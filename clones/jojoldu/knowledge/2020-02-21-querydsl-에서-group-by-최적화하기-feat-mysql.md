---
topic: "Querydsl 에서 Group by 최적화하기 (feat. MySQL)"
source_type: other
source_url: "https://jojoldu.tistory.com/477"
authorship: self
...
>
> 인덱스로 인해서 이미 컬럼들이 정렬 된 상태이기 때문입니다.
...
- `order by null` 없는 경우: 약 3.86 sec
- `order by null` 추가된 경우: 약 2.59 sec (쿼리 캐시 때문에 비슷한 양의 다른 테이블로 조회)

**인덱스가 없는 컬럼으로 수행시** **유의미한** **성능** 향상이 있었습니다.
...
먼저 `OrderByNull` 이 없는 채로 테스트 해보면 group by만 추가된 쿼리가 발생합니다. 

바로 `OrderByNull` 를 추가해서 테스트를 돌려보면 `order by null` 쿼리가 잘 추가되었음을 알수 있습니다.
