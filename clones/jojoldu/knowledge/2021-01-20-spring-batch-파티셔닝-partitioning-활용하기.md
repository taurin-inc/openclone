---
topic: "Spring Batch 파티셔닝 (Partitioning) 활용하기"
source_type: other
source_url: "https://jojoldu.tistory.com/550"
authorship: self
...
![intro1](https://t1.daumcdn.net/cfile/tistory/998091366008C79708)
...
기본 구현은 SimplePartitioner로, 빈 Step Executions을 생성합니다.
...
![test1](https://t1.daumcdn.net/cfile/tistory/99BEFA356008C79708)
...
`ProductIdRangePartitioner`를 통해 `stepExecutionContext` 에 `minId`와 `maxId`가 등록 되어있으니, 이 값을 사용합니다.
...
테스트 코드를 수행해보면 다음과 같이 **ItemReader가 파티셔닝** 되어 실행되고 있음을 확인할 수 있습니다.
...
Step 로그 역시 gridSize(`5`)에 맞게 호출되고 있음을 확인할 수 있습니다.
...
파티셔닝된 Worker Step은 **그 하나로 독립적인 완전한 Step** 입니다. 페이징 처리가 필요하면 다른 Step과 마찬가지로 Worker Step 내부에서 pageSize 단위로 페이징 처리가 됩니다.

**테스트 코드**
```java
@SpringBatchTest
@TestPropertySource(properties = "chunkSize=5") // chunkSize 5 지정
...
테스트를 수행해보면 페이징 처리가 되어 각 페이지마다 5개씩 처리되는 것을 확인할 수 있습니다.
