---
topic: "MySQL Update Subquery 성능 비교 (ver.5.6)"
source_type: other
source_url: "https://jojoldu.tistory.com/522"
authorship: self
published_at: 2020-09-01
---
# MySQL Update Subquery 성능 비교 (ver.5.6)

**by 향로 (기억보단 기록을) | 2020. 9. 1.**
...
![subquery_noindex_plan](https://t1.daumcdn.net/cfile/tistory/99FF2A4C5F4DC42F38)
...
서브쿼리 최적화가 안된 것을 볼 수 있는 id 값을 보면 **두 라인이 서로 다릅니다**. 이는 두 쿼리 (메인과 서브쿼리)가 조인이 아닌 서브쿼리로 실행되었음을 의미합니다.

**수행시간**

![subquery_noindex_time](https://t1.daumcdn.net/cfile/tistory/99F5BD405F4DC42F01)

총 **6분 35초** 가 걸렸습니다. 겨우 100만건 중 1000건을 Update하는데 6분이 넘는 시간이 걸린것이죠.

### 1-2. Index

동일한 Update쿼리를 **인덱스가 있는 서브테이블** 에 사용하게 되면 실행 계획은 다음과 같습니다.

**실행계획**

![subquery_index_plan](https://t1.daumcdn.net/cfile/tistory/9926524B5F4DC42F33)
...
![subquery_index_time](https://t1.daumcdn.net/cfile/tistory/9932C03E5F4DC42F34)
...
![join_noindex_plan](https://t1.daumcdn.net/cfile/tistory/9932B04D5F4DC42F33)

이 실행계획을 해석하면
*   첫번째 row가 **서브테이블** 입니다. 즉, 서브 테이블을 table full scan (`type=ALL`) 하고 그 결과로 메인 테이블을 하나씩 바로 찍어서 (`type=eq_ref`) 메인 쿼리를 수행했음을 의미합니다.
...
![join_noindex_time](https://t1.daumcdn.net/cfile/tistory/9932FC4D5F4DC42F33)

**0.097초** 라는 말도안되는 속도로 개선이 되었습니다.
...
![join_index_plan](https://t1.daumcdn.net/cfile/tistory/9954923F5F4DC43031)

*   서브쿼리가 전체 데이터를 사용하므로 **index full scan** (`type=index`) 이 발생하였습니다.

**수행시간**

![join_index_time](https://t1.daumcdn.net/cfile/tistory/999CB6415F4DC42F30)

수행시간은 **0.06초** 로 서브테이블에 인덱스가 없는 경우와 거의 비슷합니다.
...
꼭 이렇게 실험을 하지 않더라도, 이미 MySQL 공식문서에서는 Update/Delete 서브쿼리에 대해 주의점을 언급하고 있습니다.

![docs](https://t1.daumcdn.net/cfile/tistory/99C983495F4DC42F35)
