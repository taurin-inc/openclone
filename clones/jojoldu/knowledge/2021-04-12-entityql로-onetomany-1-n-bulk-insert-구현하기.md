---
topic: "EntityQL로 OneToMany (1:N) Bulk Insert 구현하기"
source_type: other
source_url: "https://jojoldu.tistory.com/560"
authorship: self
published_at: 2021-04-12
---
# EntityQL로 OneToMany (1:N) Bulk Insert 구현하기

**by 향로 (기억보단 기록을) | 2021. 4. 12.**
...
SimpleJdbcInsert 의 `executeBatch` 가 **return 타입에 특정 컬럼 지정이 안됩니다.** return 타입으로 int[]만 가능하여 반영된 row 수만 받을 수 있습니다.
...
- **AcademyAndStudentBulkRepository**: OneToMany Bulk Insert를 수행할 Repository 입니다.
- **AcademyMatcherRepository**: Bulk Insert 되고 return 된 Academy.id를 통해 Academy.UUID를 조회합니다.
- **AcademyUniqueMatcher**: AcademyMatcherRepository를 통해 조회된 Academy.UUID 를 통해 Academy와 Student 간 연관관계를 맺어줍니다.
...
주의하실 점은 `insertStudents` 메소드의 `if(!insert.isEmpty())` 입니다. 이게 없으면, **student가 없는 경우** `insert into values` 만 쿼리가 수행되어 **Exception** 이 발생합니다.
...
AWS Aurora MySQL 5.7 r5.large 환경에서 1만개의 Academy와 1만개의 Student를 BulkInsert 하는 테스트를 진행했습니다.
...
JpaRepository에 비해 **수십배** 빠른 성능을 보여줍니다. 데이터가 늘어날수록 그 차이는 수백배까지도 발생합니다.

## 4. ManyToMany

ManyToMany는 코드 복잡도가 너무 높아집니다. 중간 매핑 테이블까지 BulkInsert 대상으로 만들어야 하며, 키 매칭 로직이 복잡해집니다.

따라서 ManyToMany 관계에서는 JpaRepository를 사용하고, 대량 데이터 처리가 필요한 단일 Entity나 OneToMany 환경에서만 EntityQL을 선택적으로 사용하는 것이 효율적입니다.

## 5. 마무리

OneToMany 환경에서의 Bulk Insert는 설정과 구현 과정이 복잡하기 때문에 모든 Entity에 적용하기보다는 **하루 발생 데이터가 수백만 건 이상인 경우** 등 성능 최적화가 필수적인 곳에 한해 적용하는 것을 권장합니다.
