---
topic: "검증부 (assert / expect)는 하드코딩한다"
source_type: other
source_url: "https://jojoldu.tistory.com/615"
authorship: self
...
둘의 차이는 **검증문 (expected)를 하드 코딩했느냐, 로직을 사용해서 구현했느냐** 의 차이가 있다.
...
> 아래 예제 코드는 단위 테스트 에서 나온 코드를 사용하였다.
...
내가 검증하고자 하는 로직에서 버그이 있는지 아닌지를 판별하는 것이 테스트 코드인데,
