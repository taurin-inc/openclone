...
1. If `$1` is empty, tell the user the usage is `/openclone:use <clone-name>` and list available clones by running:
   ```bash
...
2. Find the clone file at `~/.openclone/clones/$1.md`.
   - If it does not exist: tell the user `$1` was not found and suggest `/openclone:list`. Stop.
...
4. Read the clone file briefly to confirm the frontmatter (`display_name`, `categories`, `primary_category`, `tagline`). Report in one or two lines:
   > Active clone set to **{display_name}** ({primary_category or categories[0]} lens, categories: {categories}). Your next message will be answered as this clone. Run `/openclone:stop` to deactivate.
